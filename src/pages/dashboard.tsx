...
import { useRoqPlatformUserProfiles, useProjectsCount } from 'lib/roq';

import { ProjectsListPage as ShowCaseList } from 'pages/projects';

function DashboardPage() {
  const { session } = useSession();

  const projectsStat = useProjectsCount();

  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    {
      title: 'Projects',
      result: projectsStat?.data,
      isLoading: projectsStat.isLoading,
