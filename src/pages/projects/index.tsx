...
import { useRoqClient, useProjectsFindManyWithCount } from 'lib/roq';
import { convertQueryToPrismaUtil } from 'lib/utils';

type ColumnType = ColumnDef<ProjectsInterface, unknown>;
...
export function ProjectsListPage(props: ProjectsListPageProps) {
  const {
    filters = {},
    titleProps = {},
  ...
  const { data, error, isLoading, mutate } = useProjectsFindManyWithCount({
  ...
  const handleDelete = async (id: string) => {
    setDeleteError(null);
    try {
      await roqClient.projects.delete({ where: { id } });
      await mutate();
    } catch (error) {
  ...
  const handleView = (row: ProjectsInterface) => {
    if (hasAccess('projects', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      ...
      ? {
          ...
          cell: ({ row: { original: record } }: any) => (
            <Flex justifyContent="flex-end">
              ...
              {hasAccess('projects', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
              ...
              {hasAccess('projects', AccessOperationEnum.DELETE, AccessServiceEnum.PROJECT) && (
                <IconButton
                  onClick={(e) => {
                    e.stopPropagation();
    ...
    <Flex direction="column" gap={{ md: 6, base: 7 }} shadow="none">
      <Flex justifyContent={{ md: 'space-between' }} direction={{ base: 'column', md: 'row' }} gap={{ base: '28px' }}>
        <Flex alignItems="center" gap={1}>
          ...
          <AccessInfo entity="projects" />
        </Flex>

        {hasAccess('projects', AccessOperationEnum.CREATE, AccessServiceEnum.PROJECT) && (
